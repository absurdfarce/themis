...
public class Themis {
    ...
    private static Logger logger = LoggerFactory.getLogger(com.datastax.themis.Themis.class);

    // ---------------------------------- Commands supported by the CLI ----------------------------------
    private static void origin(Iterator<String> bits, ThemisApp.Builder builder) {
        ...
        catch (ThemisException ve) {
            System.out.println(String.format("Error creating origin cluster, please try again: %s", ve.getMessage()));
        }
    }

    private static void target(Iterator<String> bits, ThemisApp.Builder builder) {
    ...
    private static void proxy(Iterator<String> bits, ThemisApp.Builder builder) {
    ...
    throws ThemisException {
        ...
        if (first.equals("-a")) {
            ...
            try { address = InetAddress.getByName(addressStr); }
                ...
                throw new ThemisException(String.format("Exception parsing inet address %s", addressStr), uhe);
            }
            String portStr = args.next();
            int port = -1;
            try { port = Integer.parseInt(portStr); }
            catch (NumberFormatException nfe) {
                throw new ThemisException(String.format("Exception parsing port %s", portStr), nfe);
            }
    ...
    throws ThemisException {
        ...
        ThemisApp.Builder builder = ThemisApp.builder();

        while (true) {
            try { line = reader.readLine().toLowerCase(); }
            catch (IOException ioe) {
                throw new ThemisException("Exception reading next line", ioe);
            }
